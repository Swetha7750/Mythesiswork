...
#define NMEA_MAX_LENGTH 512       /*!< Set the number of consecutive and identical characters received by receiver which defines a UART pattern*/

#define BUF_SIZE (6044)
...
void esp_now_send_cb(const uint8_t *mac_addr, esp_now_send_status_t status) { //espnow
    ESP_LOGI(TAGG, "Data sent to: " MACSTR ", Send status: %s, Return code: %d", 
...
char nmea_buffer[NMEA_MAX_LENGTH]; // Buffer for one NMEA sentence
int nmeaindex = 0;
        ...
        if (xQueueReceive(uart0_queue, (void *)&event, (TickType_t)portMAX_DELAY)) {
            ...
            case UART_DATA:
                ...
                while (false) {
                        ...
                        len = uart_read_bytes(EX_UART_NUM, dtmp, BUF_SIZE, 100 / portMAX_DELAY);
                        if (len > 0) {
                            for (int i = 0; i < len; i++) {
                                char c = dtmp[i];

                                // Check if we are starting a new sentence
                                if (c == '$') {
                                    // If we were already collecting a sentence, print it
                                    if (sentence_started) {
                                        sentence_buffer[sentence_index] = '\0';  // Null-terminate the string
                                        ESP_LOGI(TAG, "%s", sentence_buffer);    // Print the complete sentence
                                        // Send the data via ESP-NOW
                                        ESP_LOGI(TAGG, "Sending data to receiver...");
                                        esp_err_t send_result = esp_now_send(receiver_mac, (uint8_t *)sentence_buffer, sentence_index+1);
                                        if (send_result == ESP_OK) {
                                            ESP_LOGI(TAGG, "ESP-NOW Send initiated successfully");
                                        } else {
                                            ESP_LOGE(TAGG, "Failed to send data: %s", esp_err_to_name(send_result));
                                        }
                                        uart_write_bytes(EX_UART_NUM, sentence_buffer, sentence_index);
                                    }

                                    // Start collecting a new sentence
                                    sentence_started = true;
                                    sentence_index = 0;
                                }


                                // Collect the sentence characters
                                if (sentence_started) {
                                    sentence_buffer[sentence_index++] = c;
                                    if (sentence_index >= BUF_SIZE - 1) {
                                        // If the sentence is too long, reset the buffer
                                        ESP_LOGI(TAG, "a");
            ...
            case UART_PATTERN_DET:
                ...
                ESP_LOGI(TAG, "[UART PATTERN DETECTED] pos: %d, buffered size: %zu", pos, buffered_size);
                if (pos == -1) {
                    ...
                    // Send data out via ESP-NOW
                    // ESP_LOGI(TAGG, "Sending data to receiver...");
                    esp_err_t send_result = esp_now_send(receiver_mac, (uint8_t *)dtmp, pos);
                    if (send_result == ESP_OK) {
                        ESP_LOGI(TAGG, "ESP-NOW send ok");
                    } else {
                        ESP_LOGE(TAGG, "ESP-NOW send error: %s", esp_err_to_name(send_result));
                    }
